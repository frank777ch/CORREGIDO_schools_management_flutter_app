...
$teacher_id = 0;
$name = $subject = $time_of_class = "";
$name_err = $subject_err = $time_of_class_err = $teacher_id_err = "";

// Check if there are teachers
$result = mysqli_query($link, "SELECT * FROM teachers WHERE school_id='{$_SESSION['id']}'");
if (mysqli_num_rows($result) <= 0) {
    echo "<script>
    alert('There are no teachers in this school');
    window.location.href='main-insert-page.php';
    </script>";
    exit; // ensure script stops here if no teachers are found
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["time_of_class"]))) {
        $time_of_class_err = "Please insert time of class.";
    } else {
        $time_of_class = trim($_POST["time_of_class"]);

        // Validate time duration (max 6 hours)
        $time_of_class_in_hours = (float)$time_of_class;
        if ($time_of_class_in_hours > 6.0) {
            $time_of_class_err = "Maximum duration for a class is 6 hours.";
        }
    }
    ...
    if (empty(trim($_POST["teacher_id"]))) {
        ...
        // Check if the teacher teaches this subject at this school
        $result1 = mysqli_query($link, "SELECT id FROM teachers WHERE id='{$teacher_id}' AND school_id='{$_SESSION['id']}' AND subject='{$subject}'");
        if (mysqli_num_rows($result1) <= 0) {
            $teacher_id_err = "The teacher does not teach this subject in this school.";
            $teacher_id = 0;
        }
    }

    // Check input errors before inserting in database
    if (empty($name_err) && empty($subject_err) && empty($time_of_class_err) && empty($teacher_id_err)) {
        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                $success_message = "Se ha registrado correctamente el grupo.";

                // Redirect to create-class.php after successful registration
                header("location: create-class.php?success_message=" . urlencode($success_message));
                exit();
            } else {
                        ...
                        <div class="card-body">
                            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                                <div class="form-group <?php echo (!empty($name_err)) ? 'has-error' : ''; ?>">
                                    ...
                                    <input type="text" name="name" class="form-control" value="<?php echo htmlspecialchars($name); ?>" placeholder="Enter Name of class">
                                    <span class="help-block" style="color:red"><?php echo $name_err; ?></span>
                                </div>
                                <div class="form-group <?php echo (!empty($subject_err)) ? 'has-error' : ''; ?>">
                                    <label>Subject</label>
                                    <input type="text" name="subject" class="form-control" value="<?php echo htmlspecialchars($subject); ?>" placeholder="Enter Subject">
                                    <span class="help-block" style="color:red"><?php echo $subject_err; ?></span>
                                </div>
                                <div class="form-group <?php echo (!empty($time_of_class_err)) ? 'has-error' : ''; ?>">
                                    <label>Time of Class</label>
                                    <input type="text" name="time_of_class" class="form-control" value="<?php echo htmlspecialchars($time_of_class); ?>" placeholder="Enter Time of class">
                                ...
                                <div class="form-group <?php echo (!empty($teacher_id_err)) ? 'has-error' : ''; ?>">
                                    ...
                                    <input type="text" name="teacher_id" class="form-control" value="<?php echo htmlspecialchars($teacher_id); ?>" placeholder="Enter Teacher ID">
                                    <span class="help-block" style="color:red"><?php echo $teacher_id_err; ?></span>
                                </div>
                                <div class="form-group">
                                    <input type="submit" class="btn btn-primary" value="Submit">
                                    <a href="main-insert-page.php" class="btn btn-link">Cancel</a>
                                </div>
                            </form>
                        </div>
                    </div>
